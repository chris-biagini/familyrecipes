...
class StepTest < Minitest::Test
  def test_valid_with_ingredients_and_instructions
    step = Mirepoix::Step.new(
      tldr: 'Mix dough',
      ingredient_list_items: [Mirepoix::Ingredient.new(name: 'Flour', quantity: '250 g')],
  ...
  def test_valid_with_ingredients_only
    step = Mirepoix::Step.new(
      ...
      ingredient_list_items: [Mirepoix::Ingredient.new(name: 'Salt')],
      instructions: nil
    )
  ...
  def test_raises_on_blank_tldr
    assert_raises(ArgumentError) do
      Mirepoix::Step.new(
        tldr: '  ', ingredient_list_items: [Mirepoix::Ingredient.new(name: 'Salt')],
        instructions: 'Season.'
      )
    end
  end
  ...
  def test_raises_when_no_ingredients_and_no_instructions
    assert_raises(ArgumentError) do
      Mirepoix::Step.new(tldr: 'Empty step', ingredient_list_items: [], instructions: nil)
    end
  end

  def test_raises_when_no_ingredients_and_blank_instructions
    assert_raises(ArgumentError) do
      Mirepoix::Step.new(tldr: 'Empty step', ingredient_list_items: [], instructions: '   ')
    end
  end

  def test_derives_ingredients_and_cross_references
    flour = Mirepoix::Ingredient.new(name: 'Flour', quantity: '500 g')
    xref = Mirepoix::CrossReference.new(target_title: 'Pizza Dough')
    salt = Mirepoix::Ingredient.new(name: 'Salt')
