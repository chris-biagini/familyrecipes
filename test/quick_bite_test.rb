...
class QuickBiteTest < Minitest::Test
  ...
  def test_parses_category
    qb = FamilyRecipes::QuickBite.new(text_source: 'Toast: Bread, butter', category: 'Breakfast')

    assert_equal 'Breakfast', qb.category
  end

  def test_generates_id_from_title
    qb = FamilyRecipes::QuickBite.new(text_source: 'Grilled Cheese: Bread, cheese, butter', category: 'Sandwiches')

    assert_equal 'grilled-cheese', qb.id
  end

  def test_parses_ingredients_separated_by_commas
    qb = FamilyRecipes::QuickBite.new(text_source: 'Salad: Lettuce, tomato, cucumber', category: 'Sides')
  ...
  def test_explicit_comma_separated_ingredients
    qb = FamilyRecipes::QuickBite.new(text_source: 'Hummus with Pretzels: Hummus, Pretzels', category: 'Snacks')
  ...
  def test_handles_no_colon
    qb = FamilyRecipes::QuickBite.new(text_source: 'Fresh Fruit', category: 'Snacks')
  ...
  def test_all_ingredient_names_returns_unique
    qb = FamilyRecipes::QuickBite.new(text_source: 'Duplicate: Apple, apple, Apple', category: 'Snacks')
  ...
  def test_strips_whitespace_from_ingredients
    qb = FamilyRecipes::QuickBite.new(text_source: 'Test:   Item One  ,  Item Two  ', category: 'Test')
  ...
  def test_stores_text_source
    ...
    qb = FamilyRecipes::QuickBite.new(text_source: text, category: 'Test')
  ...
  def test_ingredients_with_quantities_names_match_all_ingredient_names
    qb = FamilyRecipes::QuickBite.new(text_source: 'Salad: Lettuce, Tomato', category: 'Sides')
    iwq = qb.ingredients_with_quantities
