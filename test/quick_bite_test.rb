...
class QuickBiteParserTest < Minitest::Test
  def test_parses_title_from_text
    qb = Mirepoix::QuickBite.new(text_source: 'PB&J: Peanut butter, jelly, bread', category: 'Sandwiches')

    assert_equal 'PB&J', qb.title
  end

  def test_parses_category
    qb = Mirepoix::QuickBite.new(text_source: 'Toast: Bread, butter', category: 'Breakfast')

    assert_equal 'Breakfast', qb.category
  end

  def test_generates_id_from_title
    qb = Mirepoix::QuickBite.new(text_source: 'Grilled Cheese: Bread, cheese, butter', category: 'Sandwiches')

    assert_equal 'grilled-cheese', qb.id
  end

  def test_parses_ingredients_separated_by_commas
    qb = Mirepoix::QuickBite.new(text_source: 'Salad: Lettuce, tomato, cucumber', category: 'Sides')
  ...
  def test_does_not_split_on_and
    ...
    qb = Mirepoix::QuickBite.new(text_source: 'Snack: Salt and Vinegar Chips', category: 'Snacks')
  ...
  def test_does_not_split_on_with
    ...
    qb = Mirepoix::QuickBite.new(text_source: 'Crackers: Crackers with Everything Seasoning', category: 'Snacks')
  ...
  def test_explicit_comma_separated_ingredients
    qb = Mirepoix::QuickBite.new(text_source: 'Hummus with Pretzels: Hummus, Pretzels', category: 'Snacks')
  ...
  def test_handles_no_colon
    qb = Mirepoix::QuickBite.new(text_source: 'Fresh Fruit', category: 'Snacks')
  ...
  def test_all_ingredient_names_returns_unique
    qb = Mirepoix::QuickBite.new(text_source: 'Duplicate: Apple, apple, Apple', category: 'Snacks')
  ...
  def test_strips_whitespace_from_ingredients
    qb = Mirepoix::QuickBite.new(text_source: 'Test:   Item One  ,  Item Two  ', category: 'Test')
  ...
  def test_stores_text_source
    ...
    qb = Mirepoix::QuickBite.new(text_source: text, category: 'Test')
  ...
  def test_ingredients_with_quantities_names_match_all_ingredient_names
    qb = Mirepoix::QuickBite.new(text_source: 'Salad: Lettuce, Tomato', category: 'Sides')
    iwq = qb.ingredients_with_quantities
