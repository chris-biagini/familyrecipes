...
describe("Auth Bypass", () => {
  ...
  describe("Dev login route guard", () => {
    // The dev login route (/dev/login/:id) is guarded by a `constraints`
    // block in routes.rb that only matches when `Rails.env.local?` is true.
    // This cannot be tested from a dev server since the route is available
    // by definition. In production, the route simply does not exist.
